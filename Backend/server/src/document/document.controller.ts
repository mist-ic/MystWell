...
export class DocumentController {
  ...
  async getUploadUrl(@Req() req): Promise<{ uploadUrl: string; documentId: string; storagePath: string }> {
    const userId = req.user?.sub; // Extract user ID from token payload (adjust based on your AuthGuard)
  ...
  async completeUpload(@Req() req, @Body() uploadCompleteDto: UploadCompleteDto): Promise<Document> {
    ...
    return this.documentService.completeUpload(userId, uploadCompleteDto.documentId, uploadCompleteDto.storagePath);
  }
  ...
  @Get(':id/view-url')
  async getDownloadUrl(@Req() req, @Param('id', ParseUUIDPipe) id: string): Promise<{ downloadUrl: string }> {
    const userId = req.user?.sub;
